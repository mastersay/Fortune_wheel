...
void show_question_with_letter(std::string correctly_guessed, const std::string &question) {
    for (char letter_in_question: question) {
...
answer_question(std::vector<Player> *players, unsigned char rolling_player, const std::vector<std::string> &question,
    ...
    std::cout << "type your letter: ";
    std::string letter;
    while (std::cin >> letter && letter.size() != 1 ||
        ...
        if ((*players)[rolling_player - 49].round_money >= 6) {
            ...
            if (question[1].find(letter[0]) != std::string::npos) {
                *correctly_guessed += letter[0];
                show_question_with_letter(*correctly_guessed, question[1]);
            } else {
                std::cout << letter[0] << " <- is not in table";
                return 1;
            }
        } else {
            std::cout << "you don't have enough money, pick consonant: ";
            letter.clear();
            while (std::cin >> letter && (letter.size() != 1) ||
                   (consonants.find((char) toupper(letter[0])) == std::string::npos)) {
                std::cout << "wrong\ntype your consonant:";
